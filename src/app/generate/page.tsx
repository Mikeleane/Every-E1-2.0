...
const LENGTHS: LengthPreset[] = ["Brief","Standard","Extended","Exam pack"];
...
export default function GeneratePage() {
  ...
  async function exportDocx() {
    ...
    a.href = url; a.download = `aontas-${payload.level}.docx`; document.body.appendChild(a); a.click(); a.remove();
  ...
  return (
    <main className="p-6 space-y-6">
      ...
      <form className="grid md:grid-cols-2 gap-6" onSubmit={(e) => { e.preventDefault(); void generate(); }}>
        {/* Left: Controls */}
        <section className="space-y-4 print:hidden">
          <div className="flex items-center gap-3">
            <label className="font-medium">Public school</label>
            <input type="checkbox" className="h-5 w-5" checked={payload.isPublic}
          ...
          <div className="grid grid-cols-2 gap-4">
            <label className="space-y-1">
              ...
              <select className="border rounded-md p-2 w-full" value={payload.level}
                onChange={(e) => { const lv = e.target.value as Level; update("level", lv); setLevelIdx(LEVELS.indexOf(lv)); }}>
            ...
            <label className="space-y-1">
              ...
              <select className="border rounded-md p-2 w-full" value={payload.outputType}
            ...
            <label className="space-y-1">
              ...
              <select className="border rounded-md p-2 w-full" value={payload.outputLanguage}
            ...
            <label className="space-y-1">
              ...
              <select className="border rounded-md p-2 w-full" value={payload.examStyle}
                onChange={(e) => update("examStyle", e.target.value)}>
                {["None","Cambridge","Trinity","TOEFL","Linguaskill","APTIS"].map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </label>

            <label className="space-y-1">
              <span className="block text-sm font-medium">Length</span>
              <select className="border rounded-md p-2 w-full" value={payload.lengthPreset}
                onChange={(e) => update("lengthPreset", e.target.value as LengthPreset)}>
                {["Brief","Standard","Extended","Exam pack"].map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </label>
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea className="border rounded-md p-2 w-full h-28" placeholder="Paste source text here…"
              value={payload.sourceText} onChange={(e) => update("sourceText", e.target.value)} />
            <input type="url" className="border rounded-md p-2 w-full" placeholder="…or provide a source URL"
              value={payload.sourceUrl} onChange={(e) => update("sourceUrl", e.target.value)} />
          </div>

          <div className="grid md:grid-cols-2 gap-4">
            <label className="space-y-1">
              <span className="block text-sm font-medium">YouTube link (optional)</span>
              <input className="border rounded-md p-2 w-full" placeholder="https://www.youtube.com/watch?v=..."
                value={payload.youtubeUrl} onChange={(e) => update("youtubeUrl", e.target.value)} />
            </label>
            <label className="space-y-1">
              <span className="block text-sm font-medium">Image search (optional)</span>
              <input className="border rounded-md p-2 w-full" placeholder="keywords for Google Images"
                value={payload.imageQuery} onChange={(e) => update("imageQuery", e.target.value)} />
            </label>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <label className="space-y-1">
              ...
              <input className="border rounded-md p-2 w-full" placeholder="e.g., ecosystem, habitat, biodiversity"
            ...
            <label className="space-y-1">
              ...
              <input className="border rounded-md p-2 w-full" placeholder="e.g., past simple, conditionals"
            ...
            <div className="grid grid-cols-2 gap-2">
              {PRE_ACTIVITIES.map(a => (
                <label key={a} className="flex items-center gap-2">
                  <input type="checkbox" checked={payload.preActivities.includes(a)}
          ...
          <fieldset className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {ACTIVITIES.map(a => (
                <label key={a} className="flex items-center gap-2">
                  <input type="checkbox" checked={payload.activityTypes.includes(a)}
          ...
          <div className="flex items-center gap-3">
            ...
            <input type="checkbox" className="h-5 w-5" checked={payload.dyslexicFriendly}
              onChange={(e) => update("dyslexicFriendly", e.target.checked)} />
          </div>

          <div className="flex flex-wrap items-center gap-2">
            <button type="submit" className="rounded-md border px-3 py-2 text-sm hover:bg-gray-50 disabled:opacity-50" disabled={loading}>
              {loading ? "Generating…" : "Generate"}
            </button>
            <button type="button" className="rounded-md border px-3 py-2 text-sm hover:bg-gray-50 disabled:opacity-50"
              onClick={() => void simplifyOnce()} disabled={loading || levelIdx === 0} title="Tap to simplify one level">
