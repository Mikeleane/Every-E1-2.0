...
export async function POST(req: Request): Promise<Response> {
  ...
  if (body.meta.examStyle && body.meta.examStyle !== "None") metaLines.push(`Exam style: ${body.meta.examStyle}`);
  if (body.meta.sourceUrl) metaLines.push(`Source: ${body.meta.sourceUrl}`);
  if (body.meta.youtubeUrl) metaLines.push(`YouTube: ${body.meta.youtubeUrl}`);
  if (body.meta.imageQuery) metaLines.push(`Images: https://www.google.com/images?q=${encodeURIComponent(body.meta.imageQuery)}`);
    ...
    sections: [
      {
        ...
        children: [
          ...
          ...metaLines.map((m) => new Paragraph({ children: [new TextRun({ text: m })] })),
          new PageBreak(),
